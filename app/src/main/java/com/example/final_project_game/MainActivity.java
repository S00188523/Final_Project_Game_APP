package com.example.final_project_game;
...
public class MainActivity extends AppCompatActivity {
    ...
    private final List<Integer> colorViews = Arrays.asList(R.id.redCircle, R.id.blueCircle, R.id.yellowCircle, R.id.greenCircle);
    private final List<Integer> colorSequence = new ArrayList<>();
    private final Random random = new Random();
    private final Handler handler = new Handler(Looper.getMainLooper());
    private GameDBHelper databaseHelper;
    private int score = 0;

    private int expectedIndex = 0;
    private boolean isPlaying = false;
    private int round = 1; // Track current round
    private int sequenceLength = 4; // starting sequence length

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_main);
        databaseHelper = new GameDBHelper(this);
